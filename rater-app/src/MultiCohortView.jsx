...
function priceHcvCohort(cohort, sharedInfo, sharedFleetInfo, riskScoreResult) {
  ...
  // If a computed risk score exists from Tab 2, use its factor to override the
  // simple data-source qualifier — this is the core telematics-pricing linkage.
  // DECLINE verdicts from Tab 2 propagate as REFER with an explanation.
  if (riskScoreResult && riskScoreResult.factor != null && riskScoreResult.declines && riskScoreResult.declines.length === 0) {
    qual = {
      ...qual,
      factor: riskScoreResult.factor,
      label: `Risk-scored (Tab 2): ${riskScoreResult.profile} — factor ${riskScoreResult.factor}×`,
      fromRiskScore: true,
    };
  } else if (riskScoreResult && riskScoreResult.declines && riskScoreResult.declines.length > 0) {
    return {
      ...cohort, status: "REFER",
      referral_reason: `Auto-decline from Risk Scoring (Tab 2): ${riskScoreResult.declines.join("; ")}`,
      cohort_monthly: null, cohort_annual: null,
    };
  }
...
function priceCohort(cohort, sharedFields, sharedFleetInfo, riskScoreResult) {
  ...
  if (HCV_ASSET_CLASSES.has(cls)) return priceHcvCohort(cohort, sharedFields, sharedFleetInfo, riskScoreResult);
...
export default function MultiCohortView({ sharedFleetInfo, claimsExtractionData, riskScoreResult }) {
  ...
  const pricedCohorts = useMemo(() => {
    return cohorts.map((c) => {
      const priced = priceCohort(c, sharedFields, shared, riskScoreResult);
  ...
  }, [cohorts, sharedFields, riskScoreResult]);
                ...
                <div style={{ marginTop: "14px", padding: "12px 14px", background: "#F1ECE0", borderRadius: "6px", borderLeft: "3px solid #14213D" }}>
                  ...
                  {cohort.hcv_qualifier && (
                    <div style={{ fontSize: "0.74rem", color: "#5C6570", marginTop: "8px" }}>
                      {cohort.hcv_qualifier.fromRiskScore && (
                        <div style={{ display: "inline-block", background: "#1E9E5E", color: "#fff", borderRadius: "4px", padding: "2px 8px", fontSize: "0.70rem", fontWeight: 700, marginBottom: "4px", marginRight: "8px" }}>
                          ✓ Risk Score Active (Tab 2)
                        </div>
                      )}
                      <strong>Qualifier:</strong> {cohort.hcv_qualifier.label} · {cohort.hcv_qualifier.fromRiskScore ? "" : `Coverage: ${(cohort.hcv_qualifier.coverage * 100).toFixed(1)}% · `}Factor: {cohort.hcv_qualifier.factor?.toFixed(2)}×
